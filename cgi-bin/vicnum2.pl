...
$_ = $userguess ;
if (/^ZZ$/) { print "<h2> give up? - the answer is  $guess \n"; exit ;}
if (!(/^\s*[0-9]{3}\s*$/))  { print "<h2> Your guess was not valid. Enter a 3 digit number as a guess.  Hit the back button on the browser and try again! \n" ;
		     		exit ; }
print "<h2>","Hi $player - you have chosen the number $userguess  </h2><hr>" ; 

$cnt = (length($oldguess)/5) + 1 ;
if ($guess =~ /$_/) {  print "<h2><b>Congratulations - you guessed $guess in $cnt attempts</h2><p>";
